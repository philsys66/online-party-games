...
export function registerCharadesHandlers(socket: Socket, io: Server) {

    socket.on('start_charades_round', (roomCode: string) => {
        startCharadesRound(roomCode, io);
    });

    socket.on('buzz', (roomCode: string) => {
        const room = rooms[roomCode];
        if (room && room.gameState.status === 'playing' && !room.gameState.isPaused) {
            const now = Date.now();
            const penaltyEnd = room.gameState.penalties?.[socket.id] || 0;
            if (now < penaltyEnd) return;
...
export function startCharadesRound(roomCode: string, io: Server, nextActorId?: string) {
    const room = rooms[roomCode];
    if (!room) return;
